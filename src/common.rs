...
async fn secure_tcp_impl(conn: &mut Stream, key: &str, log_on_success: bool) -> ResultType<()> {
    // [PATCHED] Skip secure_tcp handshake - self-hosted OSS server does not support it.
    // The OSS version of hbbs/hbbr does not implement the KeyExchange response
    // that the client expects, causing "deadline has elapsed" timeout when logged in.
    // Data is still encrypted end-to-end between clients via NaCl box encryption.
    // Only the token transmission between client and server is affected,
    // which is acceptable for self-hosted private servers.
    log::info!("secure_tcp skipped (self-hosted server)");
    Ok(())
}
